"""Download potevap (potential evapotranspiration) monthly data from ISIMIP3b for water index processing.

Queries the ISIMIP API for all potevap monthly files across impact models, GCMs,
and SSP scenarios. Reports availability matrix, downloads with resume support,
and validates each file.

Usage:
    python scripts/download_water_potevap.py                  # Check availability only
    python scripts/download_water_potevap.py --download        # Download all files
    python scripts/download_water_potevap.py --download --dry-run  # Show what would download
    python scripts/download_water_potevap.py --validate        # Check existing files
"""
...
cfg = get_variable_config("potevap")
VARIABLE = cfg.name
SECTOR = cfg.sector
...
def search_all_files(query: ISIMIPQuery) -> List[DatasetInfo]:
    """Search ISIMIP API for all potevap monthly files across all combinations."""
    all_datasets = []
...
def search_exploratory(query: ISIMIPQuery):
    ...
    log("\n--- Exploratory Search: ISIMIP3b potevap (no model filter) ---")
    filters = SearchFilters(
    ...
    if results:
        ...
        log("  No ISIMIP3b potevap data found")

    log("\n--- Exploratory Search: ISIMIP2b potevap (old RCP ensemble) ---")
    ...
    if results_2b:
        ...
        log("  No ISIMIP2b potevap data found")
...
def print_availability_report(
    ...
    log("\n" + "=" * 80)
    log("ISIMIP3b Potential Evapotranspiration (potevap) Monthly Data Availability")
    log("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Download potevap (potential evapotranspiration) monthly data from ISIMIP3b")
    ...
    log("=" * 60)
    log("Potential Evapotranspiration (potevap) Monthly Data Download (ISIMIP3b)")
    log("=" * 60)
