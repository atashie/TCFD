"""Download rootmoist monthly data from ISIMIP3b for water index processing.

Queries the ISIMIP API for all rootmoist monthly files across impact models, GCMs,
and SSP scenarios. Reports availability matrix, downloads with resume support,
and validates each file.

Usage:
    python scripts/download_water_rootmoist.py                  # Check availability only
    python scripts/download_water_rootmoist.py --download        # Download all files
    python scripts/download_water_rootmoist.py --download --dry-run  # Show what would download
    python scripts/download_water_rootmoist.py --validate        # Check existing files
"""
...
def search_all_files(query: ISIMIPQuery) -> List[DatasetInfo]:
    """Search ISIMIP API for all rootmoist monthly files across all combinations."""
    all_datasets = []
    ...
    for model in IMPACT_MODELS:
        for scenario in SCENARIOS:
            ...
            if results:
                log(f"  {model} / {scenario}: {len(results)} datasets")
            else:
...
def search_exploratory(query: ISIMIPQuery):
    ...
    log("\n--- Exploratory Search: ISIMIP3b rootmoist (no model filter) ---")
    filters = SearchFilters(
    ...
    if results:
        ...
        log("  No ISIMIP3b rootmoist data found")

    log("\n--- Exploratory Search: ISIMIP2b rootmoist (old RCP ensemble) ---")
    ...
    if results_2b:
        ...
        log("  No ISIMIP2b rootmoist data found")
...
def print_availability_report(
    ...
    log("\n" + "=" * 80)
    log("ISIMIP3b Rootmoist Monthly Data Availability")
    log("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Download rootmoist monthly data from ISIMIP3b")
    ...
    log("=" * 60)
    log("Rootmoist Monthly Data Download (ISIMIP3b)")
    log("=" * 60)
