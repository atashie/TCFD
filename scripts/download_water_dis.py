"""Download dis (streamflow/discharge) monthly data from ISIMIP3b for water index processing.

Queries the ISIMIP API for all dis monthly files across impact models, GCMs,
and SSP scenarios. Reports availability matrix, downloads with resume support,
and validates each file.

Usage:
    python scripts/download_water_dis.py                  # Check availability only
    python scripts/download_water_dis.py --download        # Download all files
    python scripts/download_water_dis.py --download --dry-run  # Show what would download
    python scripts/download_water_dis.py --validate        # Check existing files
"""
...
def search_all_files(query: ISIMIPQuery) -> List[DatasetInfo]:
    """Search ISIMIP API for all dis monthly files across all combinations."""
    all_datasets = []
...
def search_exploratory(query: ISIMIPQuery):
    ...
    log("\n--- Exploratory Search: ISIMIP3b dis (no model filter) ---")
    filters = SearchFilters(
    ...
    if results:
        ...
        log("  No ISIMIP3b dis data found")

    log("\n--- Exploratory Search: ISIMIP2b dis (old RCP ensemble) ---")
    ...
    if results_2b:
        ...
        log("  No ISIMIP2b dis data found")
...
def print_availability_report(
    ...
    log("\n" + "=" * 80)
    log("ISIMIP3b Streamflow/Discharge (dis) Monthly Data Availability")
    log("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Download dis (streamflow/discharge) monthly data from ISIMIP3b")
    ...
    log("=" * 60)
    log("Streamflow/Discharge (dis) Monthly Data Download (ISIMIP3b)")
    log("=" * 60)
