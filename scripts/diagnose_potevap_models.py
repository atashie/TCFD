"""Diagnostic analysis of raw potential evapotranspiration (potevap) data per impact model.
...
Usage:
    python scripts/diagnose_potevap_models.py
    python scripts/diagnose_potevap_models.py --raw-dir data/raw/water_potevap --output reports/potevap_model_diagnostics.html
"""
...
# Unit conversion: kg/m2/s -> mm/month (for display only; output stays in raw units)
...
def load_annual_means(filepath: Path, years: list) -> np.ndarray | None:
    """Load raw monthly potevap, convert to mm/month, compute annual means for specified years.

    Returns 2D array of shape (n_years, n_sampled_cells) with annual mean PET values in mm/month.
    """
    try:
        ds = xr.open_dataset(filepath)
        var_name = "potevap" if "potevap" in ds.data_vars else list(ds.data_vars)[0]
        da = ds[var_name]
...
def load_monthly_means(filepath: Path, years: list) -> np.ndarray | None:
    """Load raw monthly potevap, convert to mm/month, return per-month means.

    Returns array of shape (12,) with mean PET for each calendar month across
    ...
    try:
        ...
        for yr in years:
            for mon in range(1, 13):
                ...
                if mask.sum() == 0:
                    continue
                idx = np.where(mask)[0]
                data = da.isel(time=idx).values  # (1, lat, lon)
                data_mm = data * KG_M2_S_TO_MM_MONTH
                data_sub = data_mm[:, ::SPATIAL_STRIDE, ::SPATIAL_STRIDE]
        ...
        print(f"  ERROR loading monthly means from {filepath.name}: {e}", flush=True)
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Diagnostic analysis of raw potevap model data")
    parser.add_argument("--raw-dir", type=Path,
                        default=Path("data/raw/water_potevap"),
                        help="Path to raw potevap data directory")
    parser.add_argument("--output", type=Path,
                        default=Path("reports/potevap_model_diagnostics.html"),
    ...
    print(f"Unit conversion: kg/m2/s x {KG_M2_S_TO_MM_MONTH:.0f} -> mm/month (display only)\n", flush=True)
    ...
    model_seasonal = {}       # model -> (12,) mean seasonal cycle
    ...
    for model in MODELS:
        ...
        for gcm in GCMS:
            for scenario in SCENARIOS:
                ...
                # Seasonal cycle (one per file)
                seasonal = load_monthly_means(fpath, SAMPLE_YEARS)
    ...
    fig1.update_layout(
        title="PET Annual Mean Distribution by Impact Model (2015-2024, all GCMs/SSPs pooled)",
        yaxis_title="Potential Evapotranspiration (mm month⁻¹)",
    ...
    fig2.update_layout(
        title="PET Annual Mean Distribution by Impact Model x GCM (2015-2024)",
        yaxis_title="Potential Evapotranspiration (mm month⁻¹)",
    ...
    fig3.update_layout(
        title="PET Value Distribution Overlap (0.5th-99.5th percentile, all GCMs/SSPs pooled)",
        xaxis_title="Potential Evapotranspiration (mm month⁻¹)",
        yaxis_title="Count",
        yaxis_type="log",
        barmode="overlay",
        height=500,
...
<html><head>
<meta charset="utf-8">
<title>Potential Evapotranspiration (potevap) Model Diagnostic Report</title>
...
<h1>Potential Evapotranspiration (potevap) Model Diagnostic Report</h1>
<p>Analysis of raw ISIMIP3b potevap data across {len(MODELS)} impact models, {len(GCMS)} GCMs, {len(SCENARIOS)} SSP scenarios.</p>
<p>Sample period: {SAMPLE_YEARS[0]}-{SAMPLE_YEARS[-1]} | Spatial subsampling: every {SPATIAL_STRIDE}th cell | Display units: mm month<sup>-1</sup></p>
<p>Unit conversion for display: raw kg m<sup>-2</sup> s<sup>-1</sup> &times; {KG_M2_S_TO_MM_MONTH:,.0f} = mm month<sup>-1</sup></p>
<p><b>Note:</b> Output file will store values in raw kg m<sup>-2</sup> s<sup>-1</sup> to match original RCP file.</p>

<div class="note">
<b>Purpose:</b> Identify systematic baseline offsets between impact models before ensemble aggregation.
If models have very different absolute PET ranges, normalization (robust z-score) may be
needed before computing cross-model ensemble statistics. Also compare seasonal cycles
to verify models capture the expected PET seasonality.
</div>
...
<p>Global spatial mean PET by calendar month, averaged across all GCMs, SSPs, and sample years.
Models should show similar seasonal phasing even if amplitudes differ.</p>
<div id="fig5"></div>
{seasonal_html}

<h2>6. Boxplot: Model x GCM Breakdown</h2>
<div id="fig2"></div>
