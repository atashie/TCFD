"""Download TWS monthly data from ISIMIP3b for water index processing.

Queries the ISIMIP API for all tws monthly files across impact models, GCMs,
and SSP scenarios. Reports availability matrix, downloads with resume support,
and validates each file.

Usage:
    python scripts/download_water_tws.py                  # Check availability only
    python scripts/download_water_tws.py --download        # Download all files
    python scripts/download_water_tws.py --download --dry-run  # Show what would download
"""
...
def search_all_files(query: ISIMIPQuery) -> List[DatasetInfo]:
    """Search ISIMIP API for all tws monthly files across all combinations."""
    all_datasets = []
...
def build_availability_matrix(
    ...
    for ds in datasets:
        ...
        # Extract GCM from filename (climate_forcing field)
        # Filename pattern: model_gcm_socialforcing_scenario_var_global_monthly_YYYY_YYYY.nc
        parts = ds.name.split("_")
        gcm = "unknown"
...
def print_availability_report(
    ...
    log("\n" + "=" * 80)
    log("ISIMIP3b TWS Monthly Data Availability")
    log("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Download TWS monthly data from ISIMIP3b")
    ...
    log("=" * 60)
    log("TWS Monthly Data Download (ISIMIP3b)")
    log("=" * 60)
