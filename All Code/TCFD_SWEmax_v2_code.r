...
ncname_gfdl = 'clm45_gfdl-esm2m_ewembi_rcp60_2005soc_co2_swe_global_monthly_2006_2099.nc4'#"clm45_gfdl-esm2m_ewembi_rcp60_2005soc_co2_burntarea_global_monthly_2006_2099.nc4"  
ncin_gfdl = nc_open(paste0(ncpath, ncname_gfdl))
nc_gfdl = ncvar_get(ncin_gfdl,"swe")	# lon, lat, time

ncname_hadgem = 'clm45_hadgem2-es_ewembi_rcp60_2005soc_co2_swe_global_monthly_2006_2099.nc4'#"clm45_gfdl-esm2m_ewembi_rcp60_2005soc_co2_burntarea_global_monthly_2006_2099.nc4"  
ncin_hadgem = nc_open(paste0(ncpath, ncname_hadgem))
nc_hadgem = ncvar_get(ncin_hadgem,"swe")	# lon, lat, time

ncname_ipsl = 'clm45_ipsl-cm5a-lr_ewembi_rcp60_2005soc_co2_swe_global_monthly_2006_2099.nc4'#"clm45_gfdl-esm2m_ewembi_rcp60_2005soc_co2_burntarea_global_monthly_2006_2099.nc4"  
ncin_ipsl = nc_open(paste0(ncpath, ncname_ipsl))
nc_ipsl = ncvar_get(ncin_ipsl,"swe")	# lon, lat, time

ncname_miroc = 'clm45_miroc5_ewembi_rcp60_2005soc_co2_swe_global_monthly_2006_2099.nc4'#"clm45_gfdl-esm2m_ewembi_rcp60_2005soc_co2_burntarea_global_monthly_2006_2099.nc4"  
ncin_miroc = nc_open(paste0(ncpath, ncname_miroc))
nc_miroc = ncvar_get(ncin_miroc,"swe")	# lon, lat, time
	...
	for(j in 1:length(nc_lon))	{
		...
		if(any(!is.na(nc_dummy) & any(nc_dummy != missing_data)))	{
			...
			for(kh in 1:(length(nc_1019)/12))	{
				new_dat = max(nc_1019[(1:12)+(12*(kh-1))])
				data1019 = c(data1019,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data2029 = NULL
			for(kh in 1:(length(nc_2029)/12))	{
				new_dat = max(nc_2029[(1:12)+(12*(kh-1))])
				data2029 = c(data2029,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data3039 = NULL
			for(kh in 1:(length(nc_3039)/12))	{
				new_dat = max(nc_3039[(1:12)+(12*(kh-1))])
				data3039 = c(data3039,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data4049 = NULL
			for(kh in 1:(length(nc_4049)/12))	{
				new_dat = max(nc_4049[(1:12)+(12*(kh-1))])
				data4049 = c(data4049,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data5059 = NULL
			for(kh in 1:(length(nc_5059)/12))	{
				new_dat = max(nc_5059[(1:12)+(12*(kh-1))])
				data5059 = c(data5059,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data6069 = NULL
			for(kh in 1:(length(nc_6069)/12))	{
				new_dat = max(nc_6069[(1:12)+(12*(kh-1))])
				data6069 = c(data6069,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data7079 = NULL
			for(kh in 1:(length(nc_7079)/12))	{
				new_dat = max(nc_7079[(1:12)+(12*(kh-1))])
				data7079 = c(data7079,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data8089 = NULL
			for(kh in 1:(length(nc_8089)/12))	{
				new_dat = max(nc_8089[(1:12)+(12*(kh-1))])
				data8089 = c(data8089,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
			data9099 = NULL
			for(kh in 1:(length(nc_9099)/12))	{
				new_dat = max(nc_9099[(1:12)+(12*(kh-1))])
				data9099 = c(data9099,
					ifelse(new_dat > 0, new_dat, new_dat))
			}
...
all_data$Hazard = "Snowpack (SWE in mm)"
...
fwrite(all_data, paste0(ncpath, "SWE_", "rcp6.0", ".csv"))


	# combining the data
all_data = rbind(fread(paste0(ncpath, "SWE_", "rcp6.0", ".csv")), fread(paste0(ncpath, "SWE_", "rcp2.6", ".csv")))
IDcol = data.frame(ID = 1:nrow(all_data))
fwrite(cbind(IDcol, all_data), paste0(ncpath, "SWE_", "allData_30MAY2022", ".csv"))

all_data = fread(paste0(ncpath, "SWE_", "allData_01JUL2022", ".csv"))
