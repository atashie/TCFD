...
#library(dataRetrieval)	# for USGS streamflow data (I think)
library(data.table)	# mostly for fread()
library(sf)
	sf::sf_use_s2(FALSE) # for problem with intersecting spherical w flat
#library(feather)	# unnecessary to save as .feather ?
library(MazamaSpatialUtils)	# for linking lat lon to HUC codes
library(lubridate)
#library(daymetr)	# for NorAm daily met data
library(ddplyr) # mostly for left_join() merging by date
...
basinDelineation_f(gageLonLat, basinATLAS_locationAndFile, dataOut_location, basinName)
...
climateDataframe = climateInputConversion_f(pathToBasinBoundaryGPKG, pathToWatershedsGPKG, basinName = 'NeuseRiver', minTempNCDF,  maxTempNCDF, pptNCDF, dataOut_location = dataOut_location)
saveRDS(climateDataframe, paste0(dataOut_location, basinName, '_processedClimateData.RData'))
	...
	variableOrderOption = 1)	# 1 = [longitude,latitude,time]
{
	...
	ncin_tmin = nc_open(minTempNCDF)
	ncin_tmax = nc_open(maxTempNCDF)
	ncin_ppt = nc_open(pptNCDF)

	ncTmin = ncvar_get(ncin_tmin, 't2m_min')
	ncTmax = ncvar_get(ncin_tmax, 't2m_max')
	ncPPT = ncvar_get(ncin_ppt, 'tp')

		# assuming all lat / lon structures are the same
	nc_lat = ncvar_get(ncin_tmin, 'latitude')
	nc_lon = ncvar_get(ncin_tmin, 'longitude')
	nc_date = startDate + timeToDaysConversion * ncvar_get(ncin_tmin, 'time') # time is days after jan 1 1990
	...
	if(multipleModels)	{
		...
		climateDataframe = data.frame(Date = nc_date, Tmin = avgTmin, Tmax = avgTmax, Tavg = avgTavg, PPT = avgPPT, PET = avgPET)
	}
...
climateInput = readRDS(paste0(climateInputsFileLoc))

gg = runHBV_f(climateInput = climateInput)


		# function for running HBV
runHBV_f = function(
...
modelCalibration_f(climateInputsFileLoc, historicStreamflowFileLoc, pathToWatershedsGPKG, dataOut_location, numberOfRuns = 1000)
	...
	perc = c(runif(500, .1, .5), (runif(500, .5, 20))))		#max flux rate from SUZ to SLZ in mm/d
{
	...
	climateInput = as.data.table(readRDS(paste0(climateInputsFileLoc)))
	...
	valRows = c(ceiling(lengthOfCalibrationInput / 3):floor(lengthOfCalibrationInput/(3/2)))
		...
		# dataframe for capture ing calibration metrics
	cal_out = data.frame(
		...
		kgeCalibration = rep(NA,numberOfRuns),
		nseCalibration = rep(NA,numberOfRuns),
		maeCalibration = rep(NA,numberOfRuns),
		rmseCalibration = rep(NA,numberOfRuns),
		biasCalibration = rep(NA,numberOfRuns),
		kgeValidation = rep(NA,numberOfRuns),
		nseValidation = rep(NA,numberOfRuns),
		maeValidation = rep(NA,numberOfRuns),
		rmseValidation = rep(NA,numberOfRuns),
		biasValidation = rep(NA,numberOfRuns)
    )
	...
	for(ii in 1:numberOfRuns){
        cal_out$k1[ii] = min(sample(k1,1), cal_out$k0[ii]*.99)
        cal_out$k2[ii] = min(sample(k2,1), cal_out$k1[ii]*.99)
        cal_out$perc[ii] = min(sample(perc,1), cal_out$uz1[ii]*.99)
	}

	for(jj in 1:numberOfRuns){
			# running HBV
		HBVoutput = runHBV_f(
			climateInput = climateAndStreamflowInput,
			cal_out$sfcf[jj],	#snowfall correction factor [-]
			cal_out$tr[jj],	#solid and liquid precipitation threshold temperature [C]
			cal_out$tt[jj],	#melt temperature [C]
			cal_out$fm[jj],	#snowmelt factor [mm/C]
			cal_out$fi[jj],	#icemelt factor [mm/C]
			cal_out$fic[jj],	#debris-covered icemelt factor [mm/C]
			cal_out$fc[jj], # field capacity 
			cal_out$lp[jj], # parameter for PET --> AET
			cal_out$beta_soils[jj], #soil moisture drainage exponent
			cal_out$k0[jj],	# storage constant of top bucket
			cal_out$k1[jj],	# storage constant of middle bucket
			cal_out$k2[jj],	# storage constant of bottom bucket	
			cal_out$uz1[jj], #max flux rate from STZ to SUZ in mm/d
			cal_out$perc[jj])  # max flux rate from SUZ to SLZ in mm/d
	
	
		cal_out$kgeCalibration[jj] = KGE(HBVoutput$Qg[calRows], HBVoutput$historicQinmm[calRows])
		cal_out$nseCalibration[jj] = NSE(HBVoutput$Qg[calRows], HBVoutput$historicQinmm[calRows])
		cal_out$maeCalibration[jj] = mse(HBVoutput$Qg[calRows], HBVoutput$historicQinmm[calRows])
		cal_out$rmseCalibration[jj] = rmse(HBVoutput$Qg[calRows], HBVoutput$historicQinmm[calRows])
		cal_out$biasCalibration[jj] = pbias(HBVoutput$Qg[calRows], HBVoutput$historicQinmm[calRows])
		cal_out$kgeValidation[jj] = KGE(HBVoutput$Qg[valRows], HBVoutput$historicQinmm[valRows])
		cal_out$nseValidation[jj] = NSE(HBVoutput$Qg[valRows], HBVoutput$historicQinmm[valRows])
		cal_out$maeValidation[jj] = mse(HBVoutput$Qg[valRows], HBVoutput$historicQinmm[valRows])
		cal_out$rmseValidation[jj] = rmse(HBVoutput$Qg[valRows], HBVoutput$historicQinmm[valRows])
		cal_out$biasValidation[jj] = pbias(HBVoutput$Qg[valRows], HBVoutput$historicQinmm[valRows])
	}
...
pathToBasinBoundaryGPKG = paste0(dataOut_location, "watershedBoundaries_", basinName, ".gpkg")
pathToWatershedsGPKG =  paste0(dataOut_location, "HydroBASINSdata_", basinName, ".gpkg")
minTempNCDF_ERA5recent = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
maxTempNCDF_ERA5recent = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
pptNCDF_ERA5recent = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
minTempNCDF_SEAS5 = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
maxTempNCDF_SEAS5 = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
pptNCDF_SEAS5 = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
minTempNCDF_CFS = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
maxTempNCDF_CFS = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
pptNCDF_CFS = 'J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\-----.nc'
...
climateInputConversion_f = function(
	...
	pptConversionFactor = NA,
	avgTempGiven = FALSE, 
	multipleModels = c(FALSE, TRUE, TRUE),	# are there multiple models that need to be stored?
	startDate = as.Date("1990-01-01"), 	# when does the clock of the netcdf start?
	timeToDaysConversion = 1,	# convert time increments to days if necessary
	optionForPET = 1, 	# 1 = PET_fromTemp modified Pen-Mon, 
	variableOrderOption = c(1,2,2))	# 1 = [longitude,latitude,time], 2 = [ ]
{
	# load relevant libraries
	library(sf)				# for geospatial data
	sf::sf_use_s2(FALSE)	# for suppressing issue with intersecting spherical w flat
...
streamflowCalibration_f = function(
	historicStreamflow_mm = 0,
	historicTmax_C = 0,
	historicTmin_C = 0,
	historicPPT_mm = 0,
	historicDates = as.Date('1900-01-01'),
	historicRH_pct = NA,
	historicWind_kmph = NA,
	historicTavg_C = NA,
	basinLatsLons_decDeg = matrix(nrow=0,ncol=2))	{
...
## Reading in and correcting the Cali FNF data
	...
	# SJF San Jaoquin River Below Friant, FNF
SJF = read.csv('https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=sjf&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16')
SJF$Date = ymd(unlist(strsplit(SJF$DATE.TIME, " "))[seq(1,nrow(SJF)*2,2)])
SJF$acrefeet_pd = as.numeric(SJF$VALUE) * 1.983
	(sum(SJF$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
SJF_latlon = c(37.004, -119.697) 


	# FOL, Folsom Lake (alternative is AMF American R at Folsom FNF)
FOL = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=FOL&SensorNums=76&dur_code=D&Start=1900-01-01&End=2022-06-01")
...
FOL_latlon = c(38.710, -121.155)


	#SHA, Shasta Dam (USBR),  FNF
SHA = read.csv('https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=SHA&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16')
SHA$Date = ymd(unlist(strsplit(SHA$DATE.TIME, " "))[seq(1,nrow(SHA)*2,2)])
SHA$acrefeet_pd = as.numeric(SHA$VALUE) * 1.983
	(sum(SHA$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
SHA_latlon = c(40.720, -122.417)

	#TNL alternative, but only has monthy FNF
	#LEW, Lewiston, reservoir inflow
LEW = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=LEW&SensorNums=76&dur_code=D&Start=1900-01-01&End=2022-06-16")
LEW$Date = ymd(unlist(strsplit(LEW$DATE.TIME, " "))[seq(1,nrow(LEW)*2,2)])
LEW$acrefeet_pd = as.numeric(LEW$VALUE) * 1.983
	(sum(LEW$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
LEW_latlon = c(40.732, -122.800)

	# ORO, Oroville Dam, FNF
ORO = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=ORO&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16")
...
ORO_latlon = c(39.540, -121.480)
 
 
	# EXC, New Exchequer-Lk McClure, FNF
EXC = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=EXC&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16")
...
EXC_latlon =  c(37.591, -120.264)
	
	# TRM, Terminus Dam, FNF
TRM = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=TRM&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16")
TRM$Date = ymd(unlist(strsplit(TRM$DATE.TIME, " "))[seq(1,nrow(TRM)*2,2)])
TRM$acrefeet_pd = as.numeric(TRM$VALUE) * 1.983
	(sum(TRM$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
TRM_latlon = c(36.416, -118.998)
	
	# PNF, Pine Flat Dam, FNF
PNF = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=PNF&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16")
PNF$Date = ymd(unlist(strsplit(PNF$DATE.TIME, " "))[seq(1,nrow(PNF)*2,2)])
PNF$acrefeet_pd = as.numeric(PNF$VALUE) * 1.983
	(sum(PNF$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
PNF_latlon = c(36.845, -119.318)

	# ISB, Isabella Dam, FNF
ISB = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=ISB&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-16")
ISB$Date = ymd(unlist(strsplit(ISB$DATE.TIME, " "))[seq(1,nrow(ISB)*2,2)])
ISB$acrefeet_pd = as.numeric(ISB$VALUE) * 1.983
	(sum(ISB$acrefeet_pd, na.rm=TRUE) / 33) / 1000000
ISB_latlon = c(35.650, -118.479)
	
	
 
	# Sacramento River at Bend Bridge
BND = read.csv("https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=BND&SensorNums=8&dur_code=D&Start=1900-01-01&End=2022-06-01")
...
if(file.exists(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\HB_upstreams_", this_FNF_data$STATION_ID[1], ".RData")))	{
	gaged_upstreams = readRDS(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\HB_upstreams_", this_FNF_data$STATION_ID[1], ".RData"))
	these_hydro_basins = unlist(gaged_upstreams)
		...
		if(these_hydro_basins %in% names(gaged_upstreams))	{
			...
			saveRDS(gaged_upstreams,
				file=paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\HB_upstreams_", this_FNF_data$STATION_ID[1], ".RData"))
		}
	}
...
# saving model performance and calib parameters
 ...
 # plotting model performance
	...
	#                       ": \n old KGE =", round(old_nse,2), "new =", round(new_nse,2)))
			...
			write.csv(model_perf, paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\calibration__", this_FNF_data$STATION_ID[1], ".csv"))
			print(jj)
			summary(model_perf)
		}
}
...
HBV_in_ls = list()
HBV_areas = NA
subbasin_ids = c("SJF","FOL",'SHA','LEW','ORO','EXC','TRM','PNF','ISB')
for(i in 1:length(subbasin_ids))	{
	HBV_in_ls[[i]] = read.csv(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\calibration__", subbasin_ids[i], ".csv"))
}
...
for(i in c(1:length(HBV_in_ls)))	{
	...
	if(i == 1)	{
		this_FNF_data = SJF; this_FNF_latlon = SJF_latlon
	} else if(i == 2)	{
		this_FNF_data = FOL; this_FNF_latlon = FOL_latlon
	} else if(i == 3) {
		this_FNF_data = SHA; this_FNF_latlon = SHA_latlon
	} else if(i == 4) {
		this_FNF_data = LEW; this_FNF_latlon = LEW_latlon
	} else if(i == 5) {
		this_FNF_data = ORO; this_FNF_latlon = ORO_latlon
	} else if(i == 6) {
		this_FNF_data = EXC; this_FNF_latlon = EXC_latlon
	} else if(i == 7) {
		this_FNF_data = TRM; this_FNF_latlon = TRM_latlon
	} else if(i == 8) {
		this_FNF_data = PNF; this_FNF_latlon = PNF_latlon
	} else if(i == 9) {
		this_FNF_data = ISB; this_FNF_latlon = ISB_latlon
	}
	...
	# plotting model performance
			...
			jpeg(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\calibration_results_", subbasin_ids[i], ".jpg"), width=800,height=400, quality=100)
			par(cex=1.3)
			plot(mopey_df_disch$Date[these_days], mopey_df_disch$Q_in_mm[these_days],
				 #ylim=c(0.001,10),
				 type='l', lwd=2, cex=2,
...
valid_data = read.csv("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\valid_dat_SVnSJ.csv")


merge_dat = merge(theSac_summary_dat, valid_data, by='wateryr')[-1,]
summary(merge_dat)

jpeg(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\validation_annual_Sacramento_.jpg"), width=600,height=600, quality=100)
par(cex=1.3)
...
jpeg(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\validation_aprJul_Sacramento_.jpg"), width=600,height=600, quality=100)
par(cex=1.3)
...
jpeg(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\validation_octMar_Sacramento_.jpg"), width=600,height=600, quality=100)
par(cex=1.3)
...
jpeg(paste0("J:\\Cai_data\\Nuveen\\surfaceWaterData_and_Output\\validation-v-calib_Sacramento_.jpg"), width=600,height=600, quality=100)
par(cex=1.3)
