...
and was **wrong**. The reference was a three-model average from a *previous simulation
round* (its own `comment` attribute said so), the model ran ~26% below it, and the missing
area factor ran ~27% high. The two cancelled.

**Prefer a test whose signature is STRUCTURED over one that yields a single scalar.**
Stratifying the residual by the suspected factor is what settled it: under the wrong
convention the residual spanned 4.08 → 0.71 tracking land fraction; under the right one it
flattened to 0.84 → 0.71. A scalar cannot show you that.
...
`isfinite`. An unguarded global sum returned 3.6e41.
...
It then divided real demand by nothing and returned 6e9 in every year. 686 cells passed
the pooled mask with their own member baseline below the threshold.
...
`pdomww` carries `unit_conversion_info` saying to use cellarea *and* contfrac, which was
**correct** and was dismissed on bad evidence.

So the rule is not "trust attributes" or "distrust attributes". It is: **an attribute is a
claim to be checked against the values.** Where CLAUDE.md says a processed file's own
...
"year-specific collapse" (wrong — excluding collapses left the maximum untouched), then a
pooled-mask defect (right, and only found by printing the single worst cell-year and its
inputs).

**Inspect the extremum directly — its location, its numerator, its denominator, its own time
...
meets the full natural flow. Neither is "the Indus value".

**Reference-site checks on a routed layer must be basin-scale aggregates**, and the expected
direction must be stated for a specific reach, not a basin name.
