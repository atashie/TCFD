...
| **A masked array's `.data` still holds `_FillValue`** | `landseamask_no-ant.nc` encodes land as `1` and everything else as fill, so `np.asarray(ds.variables["mask"][:]) > 0.5` returned **all 259,200 cells as land** (`1e20 > 0.5`). The *full* mask encodes `0`/`1` and survives the same code, so the bug appears only when you switch masks. `.filled(np.nan)` before any comparison |
| **The mask changes the answer, not just the coverage** | `FD` reads **30.25%** of observations pinned at the calendar ceiling on the full 92,889-cell ISIMIP3b mask and **2.01%** on the 65,797-cell no-Antarctica mask. Antarctica is 29% of the full mask and carries 98.85% of that censoring. A share of cells without its denominator named is not a measurement (GUARDRAILS §14) |
| **`sen==0` measured on the wrong denominator** | Over ALL FINITE cells `ID` reads 0.542; over ACTIVE cells (either slope non-zero) it reads **0.182**. Cells that never cross a threshold have both slopes correctly at 0 and are not Sen failures. The wrong denominator flipped 4 of 9 slope recommendations. And choose from the **failure modes** anyway: ols's bias needs uneven member coverage, so where `n_members` is identical everywhere ols is safe at any Sen-zero share |
| **A negative caveat published as MUST-DISCLOSE** | `generate_delivery_caveats` promotes `saturation_caveat`/`sparsity_caveat`/`resolution_caveat` on the attribute being **non-empty**, not on it asserting anything. `saturation_caveat: "none -- 0.69% at the ceiling"` puts a must-disclose caveat reading "none" into both reports. Set the attribute only where it applies; park negatives under a name **not** on `LAYER_ATTRS_EXPORTED` (`saturation_measured`) |
| **A per-layer field decided per scenario** | `recommended_slope` written as each scenario finished would have given one layer `ols_slope` in ssp126 and `sen_slope` in ssp585 — the registry carries exactly one, so the three files would have disagreed about how to read themselves. Compute every scenario before writing any file, and assert agreement across them |
| **Containment is a check no per-layer verifier can do** | On a threshold ladder each higher rung must be a strict subset of the lower: `hd30 ≥ hd35 ≥ hd40 ≥ hd45`, `FD ≥ FDm10`, and across variables `FD ≥ ID`. A flipped comparison operator or swapped threshold cannot survive it, and nine passing contract checks would not have caught it |
